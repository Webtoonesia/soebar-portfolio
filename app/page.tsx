...
export default function Home() {
  ...
  // Fungsi baru untuk menangani animasi pengunduhan Tools
  const handleToolDownload = (e: React.MouseEvent, url: string, toolName: string) => {
    e.preventDefault();
    if (!url) {
      showToast('Tautan tidak tersedia.', 'error', 'Gagal');
      return;
    }

    // Menampilkan Toast info penyiapan file
    showToast(`Menyiapkan tautan untuk ${toolName}...`, 'info', 'Mohon Tunggu');

    // Simulasi jeda animasi sebelum mengalihkan halaman (tanpa tab baru)
    setTimeout(() => {
      showToast('Tautan siap!', 'success', 'Berhasil');
      setTimeout(() => {
        window.location.href = url; // Mengalihkan di tab yang sama
      }, 1000);
    }, 1500);
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          {loadingTools ? (
            ...
            <div className="flex flex-wrap justify-center gap-4 md:gap-6">
              {/* Diubah dari <a> menjadi <button> agar fungsi onCick menangani pengunduhan */}
              {tools.map((tool) => (
                <button 
                  key={tool.id} 
                  onClick={(e) => handleToolDownload(e, tool.download_url, tool.name)}
                  className="bg-[#1f2024] border border-gray-700 hover:border-[#B58D55] rounded-xl p-4 flex items-center gap-4 w-full md:w-[320px] transition-all duration-300 group hover:-translate-y-1 shadow-lg text-left focus:outline-none"
                >
      ...
      <footer id="about" className="bg-white border-t border-gray-200 pt-16 pb-8 mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="border-t border-gray-200 pt-8 text-center">
            <p className="text-slate-Berikut adalah perbaikan kode `app/page.tsx` untuk bagian Tools & Resources. Saya telah menambahkan fungsi `handleToolDownload` yang akan mencegah tautan terbuka di tab baru secara langsung, menampilkan animasi/notifikasi *toast* (seperti halaman detail), lalu mengalihkan pengguna ke tautan tersebut di tab yang sama setelah jeda waktu (simulasi *processing*).

Silakan salin dan timpa seluruh kode di `app/page.tsx` dengan kode di bawah ini:
...
// Konfigurasi Supabase
const SUPABASE_URL = '[https://lfwcyavmwpjfwuiemdjv.supabase.co](https://lfwcyavmwpjfwuiemdjv.supabase.co)';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imxmd2N5YXZtd3BqZnd1aWVtZGp2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI5NzQ5ODYsImV4cCI6MjA5ODU1MDk4Nn0.nwPI9m16c0OwF0LfkLI3IeJJwf_eSdv-lkhY-TX0-vI';
const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
...
export default function Home() {
  ...
  // Fungsi baru untuk menangani klik download tools
  const handleToolDownload = (e: React.MouseEvent, tool: any) => {
    e.preventDefault(); // Mencegah link langsung terbuka
    
    if (!tool.download_url) {
      showToast('Tautan unduhan tidak tersedia.', 'error', 'Error');
      return;
    }

    // Tampilkan toast processing seperti di halaman detail
    showToast(`Menyiapkan tautan untuk ${tool.name}...`, 'info', 'Processing');

    // Simulasi delay (1.5 detik) sebelum mengalihkan halaman (tidak buka tab baru)
    setTimeout(() => {
      showToast('Mengalihkan ke halaman unduhan...', 'success', 'Success');
      setTimeout(() => {
      ...
      <header className={`fixed w-full top-0 z-50 transition-all duration-300 border-b border-gray-200 ${isScrolled ? 'bg-white/95 shadow-md' : 'bg-white/80 backdrop-blur-md'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            {/* Header Logo + Text Soebar Design */}
            <div className="flex-shrink-0 flex items-center gap-3 cursor-pointer group" onClick={(e) => handleSmoothScroll(e, 'home')}>
              <img Berikut adalah perbaikan kode untuk `app/page.tsx`. Saya telah mengubah tag `<a>` pada bagian *Tools & Resources* menjadi sebuah tombol yang akan memicu fungsi `handleToolDownload`. Fungsi ini akan menampilkan notifikasi (toast) terlebih dahulu, lalu mengarahkan unduhan di tab yang sama sesuai permintaanmu.

Silakan *copy-paste* seluruh kode di bawah ini:
          ...
          {loadingTools ? (
            ...
            <div className="flex flex-wrap justify-center gap-4 md:gap-6">
              {tools.map((tool) => (
                <button 
                  key={tool.id} 
                  onClick={(e) => handleToolDownload(e, tool.name, tool.download_url)}
                  className="bg-[#1f2024] border border-gray-700 hover:border-[#B58D55] rounded-xl p-4 flex items-center gap-4 w-full md:w-[320px] transition-all duration-300 group hover:-translate-y-1 shadow-lg text-left"
                >
                  <div className="w-12 h-12 rounded-lg bg-gray-800 flex items-center justify-center flex-shrink-0 text-[#B58D55] group-hover:bg-[#B58D55] group-hover:text-white transition-colors overflow-hidden">
                    {tool.icon_url ? (
                      <img src={tool.icon_url} alt={tool.name} className="w-full h-full object-cover" />
